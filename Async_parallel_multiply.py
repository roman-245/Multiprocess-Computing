...
async def parallel_multiply_matrices(A, B):
    global stop_multiplication
    if len(A[0]) != len(B):
    ...
    with ProcessPoolExecutor() as pool:
        ...
        result_elements = await asyncio.gather(*[asyncio.get_event_loop().run_in_executor(pool, element, index, A, B) for index in indices])
...
# Функция для остоновки процесса
def stop_multiplication_process():
    global stop_multiplication
    stop_multiplication = True

if __name__ == '__main__':

    stop_multiplication = False
    ...
    try:
        # Выполняем перемножение матриц с использованием асинхронки
        result = asyncio.run(parallel_multiply_matrices(A, B))
